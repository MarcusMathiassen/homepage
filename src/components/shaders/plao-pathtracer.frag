...
vec3 getBRDF(vec3 L, vec3 N, vec3 rd, Material m) {
    vec3 col = vec3(0);
    ...
    // Diffuse
    vec3 s = Irradiance_SphericalHarmonics(N);
    vec3 diffuse = (1.0-metallic) * m.color * s;

    // Specular
    vec3 specular = vec3(1,.7,.5) * (1.0-roughness) * pow(dot(N, L), 100.0);

    // Fernel
    vec3 F0 = vec3(0.4);
    F0 = mix(F0, m.color, metallic);
    return specular + diffuse + fresnelSchlickRoughness(dot(-rd, N), F0, roughness);
} 

vec3 radiance(v3 ro, v3 rd, s32 maxStepCount, f32 nearClip, f32 farClip) {
    vec3 acc = vec3(0);
    vec3 mask = vec3(1);
    vec3 eye = ro;
    ...
    for (int i = 0; i < BOUNCES; ++i) {
        ...
        if (hit.t < farClip) {
            ...
            Material m = materials[hit.material_id];
            int steps = hit.steps;
            float t = hit.t;
...
void main()
{
    ...
    seed = v2(
            sin(f32(uFrameIndex)) * uv.y * uRes.x + uv.x,
            cos(f32(uFrameIndex)) + uv.x * uRes.y + uv.y
        );
    ...
    for (int i = 0; i < AA; ++i)
    {
        v3 offset = v3(msaa[i] / uRes.y, 0.0);
        v3 rd = uCamMatrix * normalize(v3(uv, uZoom) + offset);
        f32 farClip = 1000.0;//distance(ro, rd * sdfArea);
        color += radiance(ro, rd, maxStepCount, nearClip, farClip);
    }
    color /= f32(AA);
    
    v3 col = texture(uTex, gl_FragCoord.xy/uRes.xy).rgb;
    if(uFrameIndex == 0) col = v3(0);
    col *= f32(uFrameIndex);
    col += color;
    col /= f32(uFrameIndex+1);
